...
import xmpp
import automateNEUPrintGUI

printerData=eval(open('mainPrinters.txt','rb').read())
NEUPrinters=['BW_Letter_8.5x11','Color_Letter_8.5x11','BW_Tabloid_11x17','Color_Tabloid_11x17']
...
class App(object):
    def __init__(self,  cpp=None, printers=None, pidfile_path=None):
    ...
    def run(self):
        process_jobs( self.cpp, self.printers)
...
def process_job( cpp, printer, job):
    ...
    try:
        ...
        if printer.name in NEUPrinters:
            automateNEUPrintGUI.run(job['title'],job['ownerId'],printer.name.replace('"',''),options)
